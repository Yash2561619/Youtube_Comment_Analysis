...
def test_model_with_vectorizer(model_artifact_path, vectorizer_path):
  # 1. Search for the latest successful run in the experiment
  ...
  try:
    # 2. Load model directly from the run's artifact path
    model_uri = f"runs:/{latest_run_id}/{model_artifact_path}"
    model = mlflow.pyfunc.load_model(model_uri)

    # 3. Load vectorizer from local path
